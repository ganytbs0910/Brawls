import { MapDetail } from '../../types/type';

export const duelMaps: Record<string, MapDetail> = {
    "暴徒のオアシス": {
    id: "du_slayers_paradise",
    name: "暴徒のオアシス",
    mode: "duel",
    image: "require('../../assets/MapImages/Slayers_Paradise.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ベル", reason: "", power: 5 },
      { name: "エリザベス", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "マンディ", reason: "", power: 5 },
      { name: "R-T", reason: "", power: 5 },
      { name: "ティック", reason: "", power: 4 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "流れ星": {
    id: "du_shooting_star",
    name: "流れ星",
    mode: "duel",
    image: "require('../../assets/MapImages/Shooting_Star.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Medium",
    recommendedBrawlers: [
      { name: "エリザベス", reason: "", power: 5 },
      { name: "マンディ", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "ナーニ", reason: "", power: 5 },
      { name: "R-T", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ベル", reason: "", power: 4 },
      { name: "ジーン", reason: "", power: 4 },
      { name: "ティック", reason: "", power: 4 },
      { name: "スプラウト", reason: "", power: 4 },
      { name: "グレイ", reason: "", power: 4 },
      { name: "Mr.P", reason: "", power: 3 },
      { name: "Max", reason: "", power: 3 },
      { name: "ペニー", reason: "", power: 3 },
      { name: "ビー", reason: "", power: 3 },
      { name: "ボニー", reason: "", power: 3 },
      { name: "グロム", reason: "", power: 3 },
    ],

    tips: [
      "実装待ち"
    ]
  },
  "禅の庭園": {
    id: "du_zen_garden",
    name: "禅の庭園",
    mode: "duel",
    image: "require('../../assets/MapImages/Zen_Garden.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "キット", reason: "", power: 5 },
      { name: "ミコ", reason: "", power: 5 },
      { name: "リリー", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "Max", reason: "", power: 5 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "果てしなき不運": {
    id: "du_infinite_doom",
    name: "果てしなき不運",
    mode: "duel",
    image: "require('../../assets/MapImages/Infinite_Doom.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Expert",
    recommendedBrawlers: [
      { name: "アンバー", reason: "", power: 5 },
      { name: "バスター", reason: "", power: 5 },
      { name: "Max", reason: "", power: 5 },
      { name: "バズ", reason: "", power: 5 },
      { name: "R-T", reason: "", power: 5 },
      { name: "クロウ", reason: "", power: 4 },
      { name: "オーティス", reason: "", power: 4 },
      { name: "バイロン", reason: "", power: 3 },
      { name: "スクウィーク", reason: "", power: 2 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "隠れ家": {
    id: "du_hideout",
    name: "隠れ家",
    mode: "duel",
    image: "require('../../assets/MapImages/Hideout.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "ペニー", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ラリー&ローリー", reason: "", power: 5 },
      { name: "グレイ", reason: "", power: 5 },
      { name: "エリザベス", reason: "", power: 5 },
      { name: "ブロック", reason: "", power: 4 },
      { name: "ストゥー", reason: "", power: 4 },
      { name: "ベル", reason: "", power: 3 },
      { name: "ジーン", reason: "", power: 3 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "不屈の精神": {
    id: "du_no_surrender",
    name: "不屈の精神",
    mode: "duel",
    image: "require('../../assets/MapImages/No_Surrender.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Expert",
    recommendedBrawlers: [
      { name: "キット", reason: "", power: 5 },
      { name: "ミコ", reason: "", power: 5 },
      { name: "リリー", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "Max", reason: "", power: 5 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "常勝街道": {
    id: "du_warriors_way",
    name: "常勝街道",
    mode: "duel",
    image: "require('../../assets/MapImages/Warriors_Way.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "ペニー", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "エリザベス", reason: "", power: 5 },
      { name: "ベル", reason: "", power: 3 },
      { name: "ジーン", reason: "", power: 3 },
      { name: "Max", reason: "", power: 2 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "猿の迷路": {
    id: "du_monkey_maze",
    name: "猿の迷路",
    mode: "duel",
    image: "require('../../assets/MapImages/Monkey_Maze.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Expert",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "エリザベス", reason: "", power: 5 },
      { name: "ストゥー", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "ブロック", reason: "", power: 4 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "スパイスプロダクション": {
    id: "du_spice_production",
    name: "スパイスプロダクション",
    mode: "duel",
    image: "require('../../assets/MapImages/Spice_Production.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "キット", reason: "", power: 5 },
      { name: "ミコ", reason: "", power: 5 },
      { name: "リリー", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "Max", reason: "", power: 5 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "ジグザグ草原": {
    id: "du_snake_prairie",
    name: "ジグザグ草原",
    mode: "duel",
    image: "require('../../assets/MapImages/Snake_Prairie.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Expert",
    recommendedBrawlers: [
      { name: "ボウ", reason: "", power: 5 },
      { name: "タラ", reason: "", power: 5 },
      { name: "バズ", reason: "", power: 5 },
      { name: "ジャネット", reason: "", power: 4 },
      { name: "グロム", reason: "", power: 4 },
      { name: "シェリー", reason: "", power: 4 },
      { name: "R-T", reason: "", power: 4 },
      { name: "ジーン", reason: "", power: 3 },
      { name: "コルト", reason: "", power: 3 },
      { name: "クロウ", reason: "", power: 2 },
      { name: "エルプリモ", reason: "", power: 1 },
      { name: "ブロック", reason: "", power: 1 },
      { name: "サンディ", reason: "", power: 1 },
    ],

    tips: [
      "実装待ち"
    ]
  },
  "大いなる入口": {
    id: "du_great_open",
    name: "大いなる入口",
    mode: "duel",
    image: "require('../../assets/MapImages/Great_Open.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Medium",
    recommendedBrawlers: [
      { name: "エリザベス", reason: "", power: 5 },
      { name: "マンディ", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "ナーニ", reason: "", power: 5 },
      { name: "R-T", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ベル", reason: "", power: 4 },
      { name: "ジーン", reason: "", power: 4 },
      { name: "ティック", reason: "", power: 4 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "グランドカナル": {
    id: "du_canal_grande",
    name: "グランドカナル",
    mode: "duel",
    image: "require('../../assets/MapImages/Canal_Grande.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ティック", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "イヴ", reason: "", power: 5 },
      { name: "ラリー&ローリー", reason: "", power: 5 },
      { name: "バーリー", reason: "", power: 5 },
      { name: "サージ", reason: "", power: 4 },
      { name: "R-T", reason: "", power: 4 },
      { name: "カール", reason: "", power: 4 },
      { name: "リリー", reason: "", power: 4 },
      { name: "ダイナマイク", reason: "", power: 4 },
      { name: "スクウィーク", reason: "", power: 4 },
      { name: "バスター", reason: "", power: 4 },
      { name: "ダリル", reason: "", power: 4 },
      { name: "リコ", reason: "", power: 4 },
      { name: "ペニー", reason: "", power: 4 },
      { name: "ジャッキー", reason: "", power: 3 },
      { name: "ラフス", reason: "", power: 3 },
      { name: "グレイ", reason: "", power: 3 },
      { name: "リリー", reason: "", power: 2 },
    ],

    tips: [
      "実装待ち"
    ]
  },
  "ミルフィーユ": {
    id: "du_layer_cake",
    name: "ミルフィーユ",
    mode: "duel",
    image: "require('../../assets/MapImages/Layer_Cake.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Hard",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "エリザベス", reason: "", power: 5 },
      { name: "ペニー", reason: "", power: 5 },
      { name: "イヴ", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "スプラウト", reason: "", power: 4 },
      { name: "ブロック", reason: "", power: 4 },
      { name: "R-T", reason: "", power: 4 },
      { name: "Max", reason: "", power: 4 },
      { name: "ストゥー", reason: "", power: 4 },
      { name: "ジーン", reason: "", power: 3 },
      { name: "カール", reason: "", power: 3 },
      { name: "ティック", reason: "", power: 3 },
      { name: "レオン", reason: "", power: 3 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "ガールズファイト": {
    id: "du_petticoat_duel",
    name: "ガールズファイト",
    mode: "duel",
    image: "require('../../assets/MapImages/Petticoat_Duel.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Medium",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "キット", reason: "", power: 5 },
      { name: "ミコ", reason: "", power: 5 },
      { name: "リリー", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "Max", reason: "", power: 5 },
      { name: "イヴ", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "四重傷": {
    id: "du_quad_damage",
    name: "四重傷",
    mode: "duel",
    image: "require('../../assets/MapImages/Quad_Damage.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Medium",
    recommendedBrawlers: [
      { name: "エリザベス", reason: "", power: 5 },
      { name: "マンディ", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "ナーニ", reason: "", power: 5 },
      { name: "R-T", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "イヴ", reason: "", power: 5 },
      { name: "ベル", reason: "", power: 4 },
      { name: "ジーン", reason: "", power: 4 },
      { name: "ティック", reason: "", power: 4 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "言い訳厳禁": {
    id: "du_no_excuses",
    name: "言い訳厳禁",
    mode: "duel",
    image: "require('../../assets/MapImages/No_Excuses.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Medium",
    recommendedBrawlers: [
      { name: "エリザベス", reason: "", power: 5 },
      { name: "マンディ", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "バイロン", reason: "", power: 5 },
      { name: "ナーニ", reason: "", power: 5 },
      { name: "R-T", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "ベル", reason: "", power: 5 },
      { name: "ブロック", reason: "", power: 4 },
      { name: "ジーン", reason: "", power: 4 },
      { name: "コルト", reason: "", power: 4 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "見えざる大蛇": {
    id: "du_shrouding_serpent",
    name: "見えざる大蛇",
    mode: "duel",
    image: "require('../../assets/MapImages/Shrouding_Serpent.png')",
    description: "実装待ち",
    recommendedBrawlers: [
      { name: "シェリー", reason: "", power: 5 },
      { name: "ボウ", reason: "", power: 5 },
      { name: "アンバー", reason: "", power: 5 },
      { name: "ブロック", reason: "", power: 5 },
      { name: "リリー", reason: "", power: 5 },
      { name: "コーデリアス", reason: "", power: 5 },
      { name: "ストゥー", reason: "", power: 5 },
      { name: "オーティス", reason: "", power: 4 },
      { name: "スクウィーク", reason: "", power: 3 },
      { name: "パム", reason: "", power: 3 }
    ],

    tips: [
      "実装待ち"
    ]
  },
  "レイヤーケーキ": {
    id: "du_layer_bake",
    name: "レイヤーケーキ",
    mode: "duel",
    image: "require('../../assets/MapImages/Layer_Bake.png')",
    description: "実装待ち",
    characteristics: ["実装待ち"],
    difficulty: "Medium",
    recommendedBrawlers: [
      { name: "ジュジュ", reason: "", power: 5 },
      { name: "ジーン", reason: "", power: 5 },
      { name: "ボウ", reason: "", power: 5 },
      { name: "スプラウト", reason: "", power: 5 },
      { name: "ガス", reason: "", power: 5 },
      { name: "アンジェロ", reason: "", power: 5 },
      { name: "イヴ", reason: "", power: 4 },
      { name: "グロム", reason: "", power: 4 },
      { name: "グレイ", reason: "", power: 4 },
      { name: "スクウィーク", reason: "", power: 3 },
      { name: "ペニー", reason: "", power: 3 }
    ],

    tips: [
      "実装待ち"
    ]
  }
};